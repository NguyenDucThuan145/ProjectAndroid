package dautien.gameversion4.ControlObjectGameClass;
...
import dautien.gameversion4.Activity.ActivityGameOver;
import dautien.gameversion4.BulletClass.Bullet;
import dautien.gameversion4.BulletClass.FireForDragon;
import dautien.gameversion4.EnemiesClass.DragonAnimation;
import dautien.gameversion4.EnemiesClass.Enemies;
import dautien.gameversion4.GameObjectClass.Element;
import dautien.gameversion4.GameObjectClass.Explosion;
import dautien.gameversion4.GameObjectClass.ParallaxBackground;
import dautien.gameversion4.ImageButtonClass.ImageButtonGun;
import dautien.gameversion4.ImageButtonClass.ImageButtonLeftAndRight;
...
public class GamePanel extends SurfaceView implements SurfaceHolder.Callback{
    ...
    public void draw(Canvas canvas){
        ...
        if(dragon != null){
            ...
            if(dragon.khoangThoiGianVoCanh == 0){playSoundFlapOfDragon();}
                    ...
                    if(stateElement == 0) {
                        try {
                            Bullet muiTen1 = new Bullet(getResources(), myElement1.mX + myElement1.getWidth(),
                                    myElement1.mY + myElement1.getHeight() / 3, R.drawable.fireball, stateElement);
                            bullets.add(muiTen1);
                        } catch (Exception e) {
    ...
    public void doDrawScore(Canvas canvas){
        Paint p = new Paint();
        p.setColor(Color.RED);
        p.setTextSize(80);
        String myScore = "SCORE   " + score;
        canvas.drawText(myScore, 600, 130, p);
    }

    public void doDrawFireBall(Canvas canvas){
        if(thoiGianDragonBan > 40){
            thoiGianDragonBan = 0;
    ...
    private void initSoundPool()  {
        ...
        this.soundElementShoot = this.soundPool.load(this.getContext(), R.raw.soundshooting, 1);
        this.soundBackGround = this.soundPool.load(this.getContext(), R.raw.background, 1);
        this.soundFlapOfDragon = this.soundPool.load(this.getContext(), R.raw.flap, 1);
    }
    ...
    public void playSoundBackGround(){
        if(soundPoolLoaded){
            ...
            this.soundPool.play(this.soundBackGround, leftVolumn, rightVolumn, 1, -1, 1f);
        }
    }

    public void playSoundFlapOfDragon(){
        if(soundPoolLoaded){
            float leftVolumn = 0.8f;
            float rightVolumn = 0.8f;
            this.soundPool.play(this.soundFlapOfDragon, leftVolumn, rightVolumn, 1, 0, 1f);
        }
    }

    public void playSoundEnemiesDie(){
        if(soundPoolLoaded){
            float leftVolumn = 0.8f;
            float rightVolumn = 0.8f;
            this.soundPool.play(this.soundEnemiesDie, leftVolumn, rightVolumn, 1, 0, 1f);
        }
    }
    ...
    public void playSoundElementShoot(){
        if(soundPoolLoaded){
            float leftVolumn = 0.8f;
            float rightVolumn = 0.8f;
            this.soundPool.play(this.soundElementShoot, leftVolumn, rightVolumn, 1, 0, 1f);
        }
    }
