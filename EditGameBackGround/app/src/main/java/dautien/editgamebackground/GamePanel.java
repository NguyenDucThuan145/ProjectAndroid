...
public class GamePanel extends SurfaceView implements SurfaceHolder.Callback{
    ...
    private int thoiGianXuatHienKT= 0;
    ...
    public void doDrawBullet(Canvas canvas){
        ...
        if(thoiGianNapDan > 10) {
            thoiGianNapDan= 0;
            Bullet muiTen1= new Bullet(getResources(), myElenment.mX, myElenment.mY, R.drawable.danlua);
            Bullet muiTen2= new Bullet(getResources(), myElenment.mX + 30, myElenment.mY + 30, R.drawable.danlua);
            Bullet muiTen3= new Bullet(getResources(), myElenment.mX - 30, myElenment.mY - 30, R.drawable.danlua);
            bullets.add(muiTen1);
            bullets.add(muiTen2);
            bullets.add(muiTen3);
        }
    ...
    public void doDrawExplosion(Bullet bullet,  Canvas canvas){
        int x= bullet.getTamX();
        int y= bullet.getTamY();
        explosion= new Explosion(getResources(), x, y);
        explosion.doDraw(canvas);
    }
            ...
            for (int i= 0; i < bullets.size(); i++)
                for(int j= 0; j < enemies.size(); j++) {
                    if (collision_B_E(bullets.get(i), enemies.get(j)) == true){
                        doDrawExplosion(bullets.get(i), canvas);
